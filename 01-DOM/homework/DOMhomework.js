...
function buildToDo(todo, index) {
  // Tu código acá:
    ...
    toDoShell.setAttribute('class', 'toDoShell') //asignar con setAttribute una clase con el nombre de todoShell
    
    let toDoText = document.createElement("span")
    toDoText.innerHTML = todo.description
    toDoText.setAttribute('id', index)//asignar como id el index.
      if(todo.complete){
        toDoText.setAttribute('class', 'completText')
      }
...
function buildToDos(toDos) {
  let newArray = toDos.map((todo, i) => buildToDo(todo, i))
  return newArray;
}
...
function displayToDos() {
  ...
  resultado.map((item)=> toDoContainer.appendChild(item))
...
document.getElementById('adButton').addEventListener('click', addToDo)
